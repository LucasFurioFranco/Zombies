...
function refreshBullets(){
	var i;
	for(i=0;i<map.nBullets;i++){
		map.refreshBullet(map.bullets[i]);
	}
}
...
Map.prototype.addBullet = function(bullet){
	this.bullets[this.nBullets] = bullet;
	this.nBullets++;
	//alert(this.bullets[this.bullets.length-1].source_player.name + 	" atirou!");
}
Map.prototype.refreshBullet = function(bullet){
	bullet.dist_rest-=8;
	if(bullet.dist_rest<=0){
		map.nBullets--;
		bullet.pos = map.bullets[map.nBullets];
	}
	else{
...
document.onmousedown = function(e){
	...
	//p1.shoot();
	//function Bullet(pos, ang, power, dist_rest, source_player)
	map.addBullet(new Bullet([players[0].position[0], players[0].position[1]], players[0].lookAt, 100, 600, players[0]));
	alert("nBullets: " + map.nBullets);
}
