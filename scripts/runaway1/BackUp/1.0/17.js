...
function populate_zombies(){
	if(zombies.length<=100){
		...
		zombies.push(new Zombie("zombie_00", 100*a, 2.5+b, [x, y], angle));
	}
}
...
function moveZombies(){
	...
	for(i=0;i<zombies.length;i++){
		...
		if(dist_pontos(zombies[i].position, players[0].position) <= 25){
			...
			if(players[0].life <= 0){
				alert("YOU ARE DEAD! WELCOME TO THE HORD!");
			}
		}
	}
}
...
Zombie.prototype.move = function(){
	...
	if(this.interest_point != null){	//Se o zumbi tem algum ponto de interesse
		if(dist_pontos(this.position, this.interest_point) > this.speed+1){	//Se o zumbi está perto o suficiente de seu ponto de interesse, ele deixa de ter interesse neste ponto
			...
			if(!map.willColideObject( [this.position[0]+dx, this.position[1]] ) 	&& 		!map.willColideObject( [this.position[0], this.position[1]+dy] )){
				this.position[0] += dx;
				this.position[1] += dy;
			}
			else{
...
document.onmousedown = function(e){
	...
	//p1.shoot();
	//function Bullet(pos, ang, power, dist_rest, source_player)
	map.addBullet(new Bullet([players[0].position[0], players[0].position[1]], players[0].lookAt+(Math.random()/10)-0.05, 15, 800, players[0]));
}
