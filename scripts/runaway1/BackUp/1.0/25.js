		...
		if(dist_pontos(this.position, this.interest_point[this.interest_point.length-1]) > this.speed+1){	//Se o zumbi está perto o suficiente de seu ponto de interesse, ele deixa de ter interesse neste ponto
			...
			if(colision == null){
				...
				if(colision.type == "zom"){	//Colidirá com outro zumbi
					if(rand>0.5){
						this.lookAt += Math.PI/3;
					}
					else{
						this.lookAt -= Math.PI/3;
					}
					this.interest_point.push([this.position[0] + Math.cos(this.lookAt)*7*this.speed, this.position[1] + Math.sin(this.lookAt)*7*this.speed]);
				}
				else{	//Colidirá com objeto
					this.lookAt = colision.lookAt - Math.PI;
					this.interest_point.push([this.position[0] + Math.cos(this.lookAt)*7*this.speed, this.position[1] + Math.sin(this.lookAt)*7*this.speed]);
				}
				//IMPORTANTE - implementar algoritmo de desviar de obstáculos
				//this.interest_point.push([this.position[0]+Math.random()*100*this.speed - 50*this.speed, this.position[1]+Math.random()*100*this.speed - 50*this.speed]);				
			}
...
function draw_GUI(){
	...
	ctxGUI.fillText(zombie_info.type + " " + zombie_info.life, 20, 200);
	ctxGUI.fillText("pos: (" + Math.floor(zombie_info.position[0]) + " " + Math.floor(zombie_info.position[1]) + ")", 20, 220);

	ctxGUI.fillText("lookAt: " + zombies[0].lookAt, 20, 260);

	if(zombie_info.seesPlayer(players[0]) == false){
		ctxGUI.font="15px Georgia";
		ctxGUI.fillText("vendo o player: nao", 20, 280);
	}
	else{
		ctxGUI.font="15px Georgia";
		ctxGUI.fillText("vendo o player: sim", 20, 280);
	}

	ctxGUI.fillText("interest_point: (" + Math.floor(zombie_info.interest_point[zombie_info.interest_point.length-1][0]) + " " + Math.floor(zombie_info.interest_point[zombie_info.interest_point.length-1][1]) + ")", 20, 240);
}