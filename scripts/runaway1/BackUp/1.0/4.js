...
function init(){
	...
	zombies[0] = new Zombie("zombie_00", 100, 1.0, [300.0, 300.0]);
...
Zombie.prototype.seesPlayer = function(player){
	if(player == null){
		...
		if(	dist_pontos(this.position, player.position) <= this.sightLenth	){
			...
			//Deste trecho do código até o fim vc só aceita... sério... ele já funciona... porque mexer?
			...
			ctxGUI.fillText("relative_angle: " + relative_angle, 20, 270);

			var dif_angle = Math.abs(Math.abs(relative_angle) - Math.abs(this.lookAt));

			ctxGUI.font="15px Georgia";
			ctxGUI.fillText("dif_angle: " + dif_angle, 20, 290);
